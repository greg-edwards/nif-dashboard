...
# ======================================================
st.set_page_config(
    page_title="Auckland Network Resilience Dashboard",
    layout="wide"
)
...
if page == "Dashboard":
    ...
    st.title("Auckland Network Resilience Dashboard")

    
    st.info(
        "This section displays network importance, vulnerability, "
        "and resilience risk outputs. "
        "Use the map and filters to explore high‑risk corridors.")
...
# ======================================================
    ...
    st.markdown(
        ...
        This dashboard presents the outputs of the **Auckland Transport
        Network Resilience Assessment – Stage 2**. It has been developed
        to support **screening‑level identification of road corridors**
        that are both strategically important to movement and vulnerable
        to disruption from natural hazards.

        ### What the tool shows

        The dashboard combines two key components:

        **Road importance**  
        Importance is derived using the updated **Network Importance
        Framework (NIF)**, which assesses the relative consequence of
        losing a road segment to movement. Importance is evaluated
        separately for:
        - general traffic,
        - public transport, and
        - freight,
        and then combined to represent overall network importance.

        **Road vulnerability**  
        Vulnerability reflects exposure to natural hazards, based on
        spatial overlap with:
        - mapped floodplains,
        - flood‑prone areas, and
        - large‑scale landslide susceptibility.

        For each road segment, vulnerability is defined by the
        **most severe hazard** affecting that segment.

        ### Risk definition

        Overall resilience risk is calculated as:

        **Risk = Importance × Vulnerability**

        This ensures that high risk is only assigned where both the
        strategic importance of a corridor and its exposure to hazards
        are elevated.

        ### Intended use

        This tool is intended as a **network‑scale screening and
        prioritisation aid**. It is not designed to predict the
        likelihood, duration, or severity of individual events.

        The outputs should be used to:
        - identify priority corridors for further investigation,
        - inform resilience planning and programme development, and
        - support discussion with subject‑matter experts.

        ### Limitations

        - Vulnerability is based on publicly available hazard
          susceptibility datasets and does not explicitly model flood
          depth, velocity, or outage duration.
        - Results are indicative and should be supplemented with local
          knowledge and site‑specific assessment for corridor‑ or
          project‑level decisions.
        - The framework is designed to be updated as improved data
          becomes available.
